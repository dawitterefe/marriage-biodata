...
// Image generation function with language support
function generatePreviewImage($god_image, $god_name, $biodata, $family_details, $contact_details, $background_image, $photo, $language = 'en', $biodata_title = 'BIODATA', $family_title = 'Family Details', $contact_title = 'Contact Details', $mr_translations, $dropdown_options)
{
    ...
    // Translate titles based on language
    $biodata_title = ($language === 'mr' && $biodata_title === 'BIODATA') ? 'बायोडाटा' : $biodata_title;
    $family_title = ($language === 'mr' && $family_title === 'Family Details') ? 'कौटुंबिक माहिती' : $family_title;
    $contact_title = ($language === 'mr' && $contact_title === 'Contact Details') ? 'संपर्क माहिती' : $contact_title;

    // Translate God names if needed (assuming they might be in English)
    $god_names_array = $god_name ? explode('|', $god_name) : [''];
    if ($language === 'mr') {
        foreach ($god_names_array as &$name) {
            $name = $name; // Add custom logic if God names need translation
        }
    }

    // Translate biodata, family, and contact data
    $biodata_data = translateData(json_decode($biodata, true), $language, $mr_translations, $dropdown_options);
    $family_data = translateData(json_decode($family_details, true), $language, $mr_translations, $dropdown_options);
    $contact_data = $contact_details ? translateData(json_decode($contact_details, true), $language, $mr_translations, $dropdown_options) : [];
    ...
    $base_font_size = 10 * $scale_factor * $scale;
    $god_name_size = 16 * $scale_factor * $scale;
    $title_size = 14 * $scale_factor * $scale;
    $line_height = 15 * $scale_factor * $scale;
...
// Helper function to translate data based on language
function translateData($data, $language, $mr_translations, $dropdown_options)
{
    $translated_data = [];
    foreach ($data as $key => $value) {
        $translated_key = ($language === 'mr' && isset($mr_translations[$key])) ? $mr_translations[$key] : $key;
        if (isset($dropdown_options[$key])) {
            $index = array_search($value, $dropdown_options[$key]['en']);
            $translated_value = ($language === 'mr' && $index !== false) ? $dropdown_options[$key]['mr'][$index] : $value;
        } else {
            $translated_value = $value; // Non-dropdown values remain as is
        }
...
function drawSection($canvas, $title, $data, $current_y, $title_size, $font_size, $font_bold, $font_regular, $padding, $text_width, $line_height, $color, $scale)
{
    ...
    $width = 2480; // Define width here for consistency
    $x = (int)(($width - ($box[2] - $box[0])) / 2);
